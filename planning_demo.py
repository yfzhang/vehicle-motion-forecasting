...
def load(grid_size, fname='demo_input.mat'):
...
sys.path.append('./mdp')
# ipdb.set_trace()
# r_inv_expanded = np.expand_dims(r_inv, axis=2)
# ALGO = 'astar'
ALGO = 'hybridastar'
if ALGO == 'astar':
    import a_star
    obj = a_star.AStar(r_inv.tolist(), 44, 20, 8, 45, 0.)
elif ALGO == 'hybridastar':
    import hybrid_astar
    # obj = hybrid_astar.HybridAStar(r_inv.tolist(), 65, 50, 0, 8, 45, 0, 1., 1., 15.*np.pi/180.)
    vel = 3.
    L = 2.5
    del_t = 0.4
    obj = hybrid_astar.HybridAStar(r_inv.tolist(), 65, 50, 180.*np.pi/180, 70, 45, 0, 1., 1., 15.*np.pi/180.,0.7,
                                    vel, L, del_t)
    # obj = hybrid_astar.HybridAStar(r_inv.tolist(), 70, 45, 0, 65, 50, 0, 1., 1., 15.*np.pi/180.,0.7)
    # obj = hybrid_astar.HybridAStar(r_inv.tolist(), 65, 50, 0, 66.5, 51.5, 0, 1., 1., 15.*np.pi/180.,0.7)
plan = obj.find_plan()
ipdb.set_trace()
# obj = a_star.AStar(r_inv[:2][:2][np.newaxis], 44, 20, 8, 45, 0.)
# obj = a_star.AStar(r_inv_expanded, 44, 20, 8, 45, 0)
# ipdb.set_trace()

r_vector = r.reshape(n_states) # convert 2D reward matrix to a 1D vector
value_vector = model.find_optimal_value(r_vector, thresh=0.005)
